...
def main(cfg: DictConfig):
    ...
    update1_data_module = GameDataModule(
        data_dir="data/raw",
    ...
    update1_data_module.prepare_data()
    update1_data_module.setup()


    # Dynamically determine input and output dimensions from one batch.
    train_loader = update1_data_module.train_dataloader()
    ...
    hidden_dim = cfg.hyperparameters.get("hidden_dim", 256)
    prior_large_sigma = cfg.hyperparameters.get("prior_large_sigma", 1.0)
    drop_rate = cfg.hyperparameters.get("drop_rate", 0.1)
    start_year = update1_data_module.start_year  # Extract from the data module
    ...
    model = HMNNModel(
        ...
        n_train=update1_data_module.train_size,  # Needed for KL scaling
        pos_weight=update1_data_module.pos_weight
    )

    #pth_checkpoint_path = "models/initial_hmnn_model.pth"
    pth_checkpoint_path = "models/updated_hmnn_model_2015.pth"
    ...
    trainer.fit(model, datamodule=update1_data_module)
