...
def main(cfg: DictConfig):
    ...
    initial_start_year = 1995
    initial_end_year = 2000
    ...
    # Create a trainer for incremental updates.
    trainer = Trainer(
    ...
    # Loop: shift the window one year at a time.
    for current_start in range(initial_start_year + 1, final_year - window_size + 2):
        current_end = current_start + window_size - 1
        print(f"\nIncremental update on window {current_start}-{current_end}")

        # Create new data module for the current window.
        ...
        # Create a new Trainer instance for this update window.
        trainer = Trainer(
            max_epochs=cfg.hyperparameters.epochs_update,
            gradient_clip_val=cfg.hyperparameters.get("gradient_clip_val", 0.5)
        )

        # Train (update) the model on the new data.
        model.window_info = f"{current_start}-{current_end}"
        trainer.fit(model, datamodule=update_data_module)

        # Update the priors with the current posterior.
        model.hmm_update_model_weights()
        print(f"Priors updated after window {current_start}-{current_end}.")

        # Save the updated model.
        os.makedirs("models", exist_ok=True)
        checkpoint_filename = f"updated_hmnn_model_{current_start}_{current_end}.pth"
